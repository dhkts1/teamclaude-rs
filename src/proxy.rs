...
/// Which failure the transport-failure arm is actually looking at, decided
/// ONCE per attempt rather than re-derived at each log/retry/recycle site.
///
...
/// connection until something explicitly recycles it.
...
pub(crate) enum TransportFailureReason {
    ...
    /// anything.
    StreamReset,
    /// Past connect, not a recognized stream reset: a genuine connection-level
    /// death, already evicted from the pool by the error itself.
    Other,
}

/// Substrings of an h2 stream-reset error's rendered `Display`, matched
/// case-insensitively across the walked `source()` chain.
///
...
/// text instead, the same way [`is_offline_error`] above matches DNS text it
/// cannot downcast to either.
///
...
/// `"stream error "` for a `Kind::Reset` regardless of which of the three
/// `Initiator`s it names ("stream error sent by user: …", "stream error
/// detected: …", "stream error received: …") and regardless of the reset
/// reason, while every other `Kind` (`GoAway`, `Reason`, `User`, `Io`) always
/// renders `"connection error …"` or something with neither phrase. One
/// marker, not per-reason string matching.
const H2_STREAM_RESET_MARKERS: [&str; 1] = ["stream error "];

/// Classify a transport failure into [`TransportFailureReason`], walking the
/// same `source()` chain [`is_offline_error`] walks, for the same reason: the
/// h2 sub-kind lives strictly in a `source()` frame, never in `reqwest::Error`'s
/// own `Display`.
pub(crate) fn classify_transport_failure(err: &reqwest::Error) -> TransportFailureReason {
    if err.is_connect() {
        return TransportFailureReason::ConnectPhase;
    }
    let mut frame: Option<&(dyn std::error::Error + 'static)> = Some(err);
    while let Some(cause) = frame {
        let rendered = cause.to_string().to_ascii_lowercase();
        if H2_STREAM_RESET_MARKERS
            .iter()
            .any(|marker| rendered.contains(marker))
        {
            return TransportFailureReason::StreamReset;
        }
        frame = cause.source();
    }
    TransportFailureReason::Other
}
    ...
    for _ in 0..max_attempts {
                ...
                None => {
                    ...
                    match soft_wait_secs(
                        ...
                        None => {
                            ...
                            //
                            // And when the held account went out DURING this request
                            // (an upstream rejection on it just turned
                            // `bound_account_holds` to no), this is the same move the
                            // top of the handler makes on a later turn, so it gets the
                            // same answer now instead of a 429 that waits for that
                            // turn: the warning once, then the fleet.
                            if let Some(bound) = bound_to {
                                if !manager.bound_account_holds(
                                    bound,
                                    now,
                                    request_is_fable,
                                    request_group.as_deref(),
                                    ...
                                    tracing::info!(
                                        account =
                                            manager.account_name(bound).as_deref().unwrap_or("?"),
        ...
        let resp = match builder.send().await {
            ...
            Err(err) => {
                ...
                if transport_reason == TransportFailureReason::StreamReset
                    && reset_recycled.insert(idx)
                {
                    // The h2 connection survives a stream reset (unlike a
...
///
/// Long enough that the warning is on screen for a moment before the retry clears it, short
/// enough that a switch, which happens once per conversation per account running out, costs
/// the user a few seconds and no action.
const SWITCH_WARNING_RETRY_AFTER_SECS: i64 = 3;

/// The one 429 a held conversation gets before tcr moves it to another account, or `None`
/// when it should just move.
///
...
fn switch_warning(
    ...
    let hint = manager.exhaustion_hint_for_account(now, is_fable, bound_idx);
    // `binding` is only the gate that TIMES a wait; a terminal one (rejected, disabled, a dead
    // login) has no clear instant and leaves it `None`. Over one account, `gated` holds that
    // account's own reason either way.
    let why = hint
        .binding
        .or_else(|| hint.gated.keys().next().copied())
    ...
    tracing::info!(
        account = manager.account_name(bound_idx).as_deref().unwrap_or("?"),
        binding = ?hint.binding,
        retry_after = SWITCH_WARNING_RETRY_AFTER_SECS,
        "held conversation's account is out; warning the client once before moving it"
    );
    Some(error_response(
        StatusCode::TOO_MANY_REQUESTS,
        "rate_limit_error",
        &format!(
            "tcr: the account holding this conversation is out ({why}{until}); the retry moves \
             it to another account, where its prompt cache starts cold."
        ),
        Some(SWITCH_WARNING_RETRY_AFTER_SECS),
    ))
}

/// What a gate means, in words a person reads after an account count or after "out".
fn gate_phrase(reason: GateReason) -> &'static str {
    match reason {
...
fn describe_gates(gated: &std::collections::BTreeMap<GateReason, usize>) -> String {
    let parts: Vec<String> = gated
        .iter()
        .map(|(reason, n)| format!("{n} {}", gate_phrase(*reason)))
        .collect();
    if parts.is_empty() {
        String::new()
    } else {
...
mod tests {
    ...
    ///
    /// The move is told first: the first request gets one plain 429 saying so, spending no
    /// account, and the client's retry of it is the request that moves (`switch_warning`).
    ///
    ...
    async fn a_bound_account_that_is_out_releases_the_conversation_to_a_sibling() {
        ...
        let turn = thinking_turn_body("sess-pinned", &["sig_fake_1"]);

        let (status, body) = drive_conversation(Arc::clone(&manager), &turn).await;
        assert_eq!(
            status,
            StatusCode::TOO_MANY_REQUESTS,
            "the first request is told the conversation is moving: {}",
            String::from_utf8_lossy(&body)
        );
        assert!(
            String::from_utf8_lossy(&body).contains("(disabled)"),
            "and why: {}",
            String::from_utf8_lossy(&body)
        );
        assert_eq!(
        ...
        assert_eq!(
            status,
            StatusCode::OK,
            "bound to an account that cannot serve, the retry goes to one that can: {}",
            String::from_utf8_lossy(&body)
        );
        assert_eq!(
            hits.load(std::sync::atomic::Ordering::SeqCst),
    ...
    async fn a_held_account_running_out_mid_request_warns_once_then_moves() {
        ...
        let turn = thinking_turn_body("sess-moving", &["sig_held"]);

        let warned = drive_conversation_full(Arc::clone(&manager), &turn).await;
        assert_eq!(warned.status(), StatusCode::TOO_MANY_REQUESTS);
        assert!(
            warned
                .headers()
                .get("anthropic-ratelimit-unified-representative-claim")
                ...
                && warned
                    ...
                    .get("anthropic-ratelimit-unified-status")
                    .is_none(),
            "the warning is a plain 429, the only kind whose text the client shows"
        );
        assert_eq!(
            warned
                .headers()
                .get("retry-after")
                .and_then(|v| v.to_str().ok()),
            Some(SWITCH_WARNING_RETRY_AFTER_SECS.to_string().as_str()),
        );
        let body = to_bytes(warned.into_body(), MAX_BODY_BYTES)
            .await
            .expect("read body");
        let text = String::from_utf8_lossy(&body);
        assert!(
            text.contains("moves it to another account")
                && text.contains("(rejected upstream)")
        ...
        // The client's retry of the warning: the same request, and it moves.
        let (status, body) = drive_conversation(Arc::clone(&manager), &turn).await;
        assert_eq!(
            status,
            StatusCode::OK,
    ...
    async fn bound_history_on_a_hold_past_the_cache_goes_to_a_sibling() {
        ...
        let turn = thinking_turn_body("sess-far", &["sig_fake_minted_before_this_build"]);
        // Told first (`switch_warning`), and the client's retry is the request that moves.
        let (status, body) = drive_conversation(Arc::clone(&manager), &turn).await;
        assert_eq!(
            status,
            StatusCode::TOO_MANY_REQUESTS,
        ...
        assert!(
            String::from_utf8_lossy(&body).contains("(on an upstream hold until "),
            "and why: {}",
            String::from_utf8_lossy(&body)
        );
        let (status, body) = drive_conversation(Arc::clone(&manager), &turn).await;
    ...
    /// Issue a real GET over cleartext HTTP/2 (prior knowledge, no TLS) and
    /// hand back the real `reqwest::Error`. Mirrors `transport_error_for` in
    /// `manager/mod.rs`'s tests: a hand-built stub would prove nothing about
    /// what the LIVE h2 error's `source()` chain actually renders.
    async fn h2_transport_error_for(url: &str) -> reqwest::Error {
    ...
    /// own h2 layer actually produces.
    async fn spawn_stream_resetting_upstream() -> SocketAddr {
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        let addr = listener.local_addr().unwrap();
        tokio::spawn(async move {
            loop {
                ...
                tokio::spawn(async move {
                    let service = hyper::service::service_fn(
                        |_req: hyper::Request<hyper::body::Incoming>| async {
                            Err::<hyper::Response<String>, _>(std::io::Error::other(
                                "reset every stream on purpose",
                            ))
                        },
                    );
                    ...
                    .serve_connection(hyper_util::rt::TokioIo::new(sock), service)
                    .await;
                });
            }
        });
    ...
    #[tokio::test]
    async fn stream_reset_classifies_distinctly_from_connect_phase() {
        let up_addr = spawn_stream_resetting_upstream().await;
        let reset = h2_transport_error_for(&format!("http://{up_addr}/v1/messages")).await;
        println!(
            "reset case: reason={:?} is_connect={} err={reset:?}",
            classify_transport_failure(&reset),
            reset.is_connect()
        );
        assert!(
            !reset.is_connect(),
            "precondition: a stream reset happens AFTER the connection exists"
        );
        assert_eq!(
            classify_transport_failure(&reset),
            TransportFailureReason::StreamReset,
            "an h2 stream reset must classify as StreamReset, not lumped in with a \
             connection-level death: {reset:?}"
        );

        let connect = h2_transport_error_for(&format!(
            "http://127.0.0.1:{}/v1/messages",
            closed_loopback_port()
        ))
        .await;
        println!(
            "connect case: reason={:?} is_connect={} err={connect:?}",
            classify_transport_failure(&connect),
            connect.is_connect()
        );
        assert!(
            connect.is_connect(),
            "precondition: nothing is listening, so this never leaves the connect phase"
        );
        assert_eq!(
            classify_transport_failure(&connect),
            TransportFailureReason::ConnectPhase,
            "a CONNECT failure must not be treated as a stream reset: {connect:?}"
        );
    }
    ...
    #[tokio::test]
    async fn recycle_client_after_reset_swaps_the_arc_only_when_called() {
        let up_addr = spawn_stream_resetting_upstream().await;
        let manager = fleet(up_addr, &["a"]);

        let before = manager.http_client(0).expect("account 0 exists");
        let reset = h2_transport_error_for(&format!("http://{up_addr}/v1/messages")).await;
        assert_eq!(
            classify_transport_failure(&reset),
            TransportFailureReason::StreamReset
        );
        manager.recycle_client_after_reset(0);
        let after_reset = manager.http_client(0).expect("account 0 exists");
        assert!(
            !Arc::ptr_eq(&before, &after_reset),
            "a stream reset's recycle must hand the next attempt a DIFFERENT client, \
             or it checks out the same broken connection again"
        );

        // A ConnectPhase failure never reaches the recycle call in the live arm
        // (mirrored here rather than re-driving the whole proxy loop): the
        ...
        let after_connect = manager.http_client(0).expect("account 0 exists");
        assert!(
            Arc::ptr_eq(&after_reset, &after_connect),
            "a CONNECT failure must never recycle a client that was never pooled"
        );
    }
