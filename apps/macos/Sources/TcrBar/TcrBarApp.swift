...
struct TcrBarApp: App {
    ...
    var body: some Scene {
        MenuBarExtra {
            FleetView(poller: poller, server: server)
                .onAppear {
                    poller.start()
...
/// The glyph in the menu bar: worst enabled account wins, so a single spent
/// account is visible without opening the panel. A failed read shows a warning
/// glyph rather than a healthy-looking gauge.
struct MenuBarLabel: View {
    ...
    var body: some View {
        ...
        case .loaded(let fleet):
            Image(systemName: Tok.glyph(for: fleet.headline))
        case .pending:
